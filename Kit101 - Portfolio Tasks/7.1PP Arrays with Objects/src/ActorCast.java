...
/**
 * Actor Cast (7.1PP Arrays of objects)
 *
 ...
 * displays the full actor details or just the most valuable actor or the
...
public class ActorCast {
  ...
  /*
   * Create an array of actors based on details entered by the user /* @param sc A
   * Scanner object
   * 
   ...
   * @param actors    Array of Actor Objects
   * @param numActors Number of actors in the actors array
   * @return highest Actor with the highest revenue potential
   */
  public static Actor mostValuableActor(Actor[] actors, int numActors) {
    Actor highest = actors[0];
    System.out.println(); // Leave a blank line
    for (int i = 0; i < numActors; i++) {
      if (actors[i].getRevenue() > highest.getRevenue()) {
        highest = actors[i];
      }
    }
    return highest;
   ...
   */
  public static int averageActorRevenue(Actor[] actors, int numActors) {
    int revenue = 0;
    int averageRevenue = 0;

    System.out.println(); // Leave a blank line
    for (int i = 0; i < numActors; i++) {
      revenue = revenue + actors[i].getRevenue(); // Sum the revenue from all actors
    }
    if (numActors > 0) { // Just in case there is nothing in the array to prevent divide by zero error
      averageRevenue = revenue / numActors; // calculate the average revenue
    }
  ...
  public static void main(String[] args) {
    ...
    do {
      ...
      System.out.println("3. Display most valuable Actor");
      System.out.println("4. Display average Actor Revenue");
      ...
      case 3: {
        highest = mostValuableActor(actors, numActors); // Calculate and Return the most valuable actor
        System.out.println("The most valuable actor is " + highest.toString());
        System.out.println();
        break;
      }
      case 4: {
        averageRevenue = averageActorRevenue(actors, numActors);// Calculate and Return the average revenue
        System.out.println("The average actor revenue is $" + averageRevenue + "M");
        System.out.println();
        break;
      }
      }
