...
/**
 * Utility for comparing the runtime of various sorting algorithms. Part of
 * 6.3DN Sorting Algorithms. V1.1 Added logic for a Bubble sort algorithm
 * 
 * 
 * 
...
public class SortTiming {
  ...
  /**
   * Create a new SortTiming object with the random number generator initialised
   * with seed.
   */
  ...
  /**
   * Generates an array of size elements randomly initialised between lo and hi,
   * inclusive.
   */
  ...
  /**
   * Compares all supported sorting algorithms on randomised data in range [lo,
   * hi].
   */
  ...
  /**
   * Compares all supported sorting algorithms on the given array of integers,
   * optionally displaying their results after to confirm they all achieve the
   * same outcome.
   */
  ...
  private void displayTime(String prefix, long time) {
    System.out
        .println(String.format("%s%d ms\tor\t%2.1f s\tor\t%2.1f min", prefix, time, time / 1000.0, time / 60000.0));
  }
  ...
  public long timeSort(int[] data, Alg alg) {
    ...
    switch (alg) {
    case BUBBLE:
      ArrayRoutines.bubbleSort(data);
    case SELECT:
      ArrayRoutines.selectionSort(data);
      break;
    case INSERT:
      ArrayRoutines.insertionSort(data);
      break;
  ...
  public static void main(String[] args) {
    ...
    // Comment or uncomment relevant line below to perform tests on random or sorted
    // data
