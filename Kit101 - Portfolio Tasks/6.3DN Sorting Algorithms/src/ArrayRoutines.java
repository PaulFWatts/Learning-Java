 ...
 * commented out to make the comparison with in-built sorting methods a _little_
 * more fair (there are still other optimisations that should be made to ensure
 * we are comparing algorithmic differences, not implementation differences)
 * V1.1 Added logic for a Bubble sort algorithm
 *
 * @author Robyn Gibson, Julian Dermoudy, James Montgomery, Paul Watts
...
public class ArrayRoutines {
  ...
  /**
   * Performs a linear search for the location of a value in the array; returns -1
   * if it is not present. See, e.g.,
   * <a href="https://en.wikipedia.org/wiki/Linear_search">the Wikipedia
   * article</a>.
   * 
   ...
   * <a href="https://en.wikipedia.org/wiki/Binary_search">the Wikipedia
   * article</a>.
   * 
   * @param data   the array to search
  ...
  /**
   * Sorts the array into ascending order using a bubble sort. See, e.g., <a
   * href=https://en.wikipedia.org/wiki/Bubble_sort>the Wikipedia article</a>.
   * 
   * @param data the array to sort
   */
  ...
  // General utility functions for displaying and using arrays
